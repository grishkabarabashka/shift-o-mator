...
/// Non-working is judged by the *person's location* calendar (ADR-0002), not the shift's.
/// </summary>
    ...
    public static ProposeResult Propose(ProposeParams p)
    {
        ...
        foreach (var assignment in ordered)
        {
            ...
            var slot = FindSlot(new SlotSearchInput(
                person.Id, assignment.Date, unit.CompOffPolicy, p.Index, p.Absences, occupied));
