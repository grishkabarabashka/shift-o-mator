...
public static class DateHelpers
{
    ...
    /// <summary>Absolute [start, end) window for a shift's duty on a date, in the shift's own
    /// timezone (ADR-0001) — mirrors engine/dates.ts#shiftInterval. DST-aware via
    /// TimeZoneInfo, not a fixed UTC offset.</summary>
    public static UtcInterval ShiftInterval(Shift shift, DateOnly date, TimeOverride? overrideWindow = null)
    {
        var start = overrideWindow?.Start ?? shift.Start;
        var end = overrideWindow?.End ?? shift.End;
        var crossesMidnight = overrideWindow?.CrossesMidnight ?? shift.CrossesMidnight;

        var tz = TimeZoneInfo.FindSystemTimeZoneById(shift.TimeZone);
        ...
        if (endUtc <= startUtc)
        {
            throw new InvalidOperationException(
                $"Role window for {shift.Code} on {date:yyyy-MM-dd} is empty or negative: {start}–{end}");
        }
