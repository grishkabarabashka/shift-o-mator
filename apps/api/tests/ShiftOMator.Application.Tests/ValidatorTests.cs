...
public class ValidatorTests
{
    ...
    private static readonly DayConfiguration Weekday = MakeDayConfig(
        ...
        roleRequirements:
        [
            new ShiftRequirement { DayConfigurationId = "", ShiftId = LeadRole.Id, Min = 1, Max = 2, IsDefault = true },
    ...
    private static List<Issue> IssuesFor(Scenario s)
    {
        ...
        var coverageCells = CoverageCalculator.Compute(TestUnit.Id, RangeFrom, RangeTo, data.Assignments, index);
        return Validator.Validate(new Validator.ValidateParams(
            TestUnit.Id, RangeFrom, RangeTo, data.Assignments, data.Absences, data.CompDays,
    ...
    public class AcknowledgeableConflicts
    {
        ...
        public void A_role_outside_the_persons_eligibility()
        {
            ...
            var issue = FirstOf(issues, IssueCode.ShiftNotEligible);
            Assert.Equal(IssueLevel.Warning, issue?.Level);
            Assert.Equal(IssueCategory.Conflict, issue?.Category);
        }
    ...
    public class Warnings
    {
        ...
        public void A_role_pool_limit_catches_what_the_region_counter_misses()
        {
            ...
            var people = new List<Person>
            {
                MakePerson("p-lead-1"),
                MakePerson("p-lead-2"),
                MakePerson("p-other-1", eligibility: [new ShiftEligibility { PersonId = "", ShiftId = NightRole.Id, TargetShare = 1 }]),
                MakePerson("p-other-2", eligibility: [new ShiftEligibility { PersonId = "", ShiftId = NightRole.Id, TargetShare = 1 }]),
            };
            ...
            Assert.All(capacity, i => Assert.Equal(LeadRole.Id, i.ShiftId));
        }
    ...
    public class Info
    {
        ...
        public void Deviation_from_a_target_role_share()
        {
            var person = MakePerson("p-1", eligibility:
            [
                new ShiftEligibility { PersonId = "", ShiftId = LeadRole.Id, TargetShare = 0.3 },
                new ShiftEligibility { PersonId = "", ShiftId = NightRole.Id, TargetShare = 0.7 },
            ]);
            ...
            Assert.Contains("actual 100% vs target 30%", deviations.First(i => i.ShiftId == LeadRole.Id).Message);
        }
