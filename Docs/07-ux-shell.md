...
```
┌──────────────────────────────────────────────────────────────────────────┐
│ ● Shift-o-mator  [Only Me]  [ALL|AMER|EMEA|APAC|ST]  ✏️Draft    ⬤ Name ▾ │  product header
├──────────────────────────────────────────────────────────────────────────┤
│  Dashboard   Schedule   People   Settings                                │  masthead
├──────────────────────────────────────────────────────────────────────────┤
│                                                                          │
...
**Product header**, left to right: product identity; **Only Me** switch; segmented
**planning unit** selector — `ALL` plus one segment per unit, so the three regions and
any cross-region team such as Service Transition sit side by side; an amber
`✏️ Draft` tag when a draft is open; spacer; user badge with avatar, display name and
role tag.

The user badge opens a popover holding identity (read-only) and the **display timezone**
control. No role switcher ships to ordinary users.

**Masthead:** Dashboard, Schedule, People, Settings. Active item visually selected.

## Global controls

| Control | Scope | Effect |
|---|---|---|
| Only Me | All person and assignment views | Restricts to the signed-in person. Answers "what am I doing?" |
| Unit selector | Dashboard, Schedule, People | `ALL` is a global overview; one unit is that unit's roster. **A default filter, not a boundary** — Schedule offers a "whole region" toggle, and everyone can write everywhere. |
| Display timezone | Timelines and all time-bearing detail | Repositions displayed windows. Never changes stored definitions. |
| Date range | Dashboard, Schedule | Back / Today / forward, zoom, presets, day strip, year minimap on long ranges. |
| Display options | Schedule | Show or hide off days and weekends; toggle gap and conflict emphasis. |

**Filters survive navigation within a session.** Jumping from a Dashboard gap to
...
| Date-range widget | Prev / Today / next, range label, zoom, presets, day strip, year minimap. Today has a red border; the selected range is blue. | Dashboard, Schedule |
| Summary statistic | Large number, small label, in one horizontal card. Color shifts on alert. | Dashboard |
| Alert row | `GAP` or `CONFLICT` badge, region, role, actual vs required | Dashboard |
| Collapsible region card | Arrow, region name, filled/required, 24h minibar with now marker; expands to timeline detail | Dashboard |
...
| Timeline lane | Region/role row with positioned shift block, gap styling, handover band, now marker | Dashboard, Timeline |
...
- Primary editing target is desktop ≥1280px; 1024px remains usable via horizontal
  scroll.
- Below 1024px prioritize read-only Dashboard, Only Me and the personal schedule. Dense
