...
| 5 | HTTP cutover: an HTTP client replaces the in-memory store, all CRUD over REST; TanStack Query for server state, Zustand only for UI state and draft metadata | Frontend talks entirely to backend over `/api/*` endpoints (the repository interface and the generated schema were later removed — ADR-0066, ADR-0067) |
