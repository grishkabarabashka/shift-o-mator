...
import type { Person, RoleEligibility, ShiftRole, Weekday } from '../../domain/types.ts';
...
function initialState(person: Person): DraftState {
  return {
    shares: new Map(
      person.eligibility.map((e) => [e.roleId, Math.round(e.targetShare * 100)]),
    ),
...
export function PersonEditor({ person, regionRoles, onClose }: Props) {
  ...
  const setShare = (roleId: string, percent: number | undefined) => {
    const shares = new Map(draft.shares);
    if (percent === undefined) shares.delete(roleId);
    else shares.set(roleId, Math.max(0, Math.min(100, percent)));
    ...
    <div className="flex flex-col gap-4">
      <section>
        <div className="flex items-baseline gap-2">
          <h3 className="menu-label px-0">Role mix for auto-populate</h3>
          <span
        ...
        <ul className="mt-1 space-y-1.5">
          {regionRoles.map((role) => {
            const percent = draft.shares.get(role.id);
            ...
            return (
              <li key={role.id} className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={eligible}
                  aria-label={`Eligible for ${role.code}`}
                  onChange={() => setShare(role.id, eligible ? undefined : 10)}
                />
                <span
                  aria-hidden
                  className="h-2.5 w-2.5 shrink-0 rounded-[3px]"
                  style={{ background: role.color, opacity: eligible ? 1 : 0.3 }}
                />
                <span
                ...
                <input
                  ...
                  value={percent ?? 0}
                  disabled={!eligible}
                  aria-label={`${role.code} target share`}
                  className="min-w-0 flex-1 accent-[var(--accent)] disabled:opacity-30"
                  onChange={(event) => setShare(role.id, Number(event.target.value))}
                />
                <span
        ...
        <p className="mt-1.5 text-[10.5px] text-faint">
          Unchecking a role removes it from this person&rsquo;s eligibility entirely.
        </p>
      </section>
...
function applyTo(person: Person, draft: DraftState): Person {
  ...
  const previous = new Map(person.eligibility.map((e) => [e.roleId, e]));

  const eligibility: RoleEligibility[] = [...draft.shares.entries()].map(([roleId, percent]) => {
    const before = previous.get(roleId);
    return {
      ...(before ?? {}),
      roleId,
