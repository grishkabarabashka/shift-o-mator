...
interface Props {
  ...
  readonly onPick: (personId: PersonId, roleId: RoleId, date: IsoDate) => void;
}
        ...
        <>
          ...
          {result.available.slice(0, 8).map((candidate, i) => (
            <button
              ...
              role="menuitem"
              onClick={() => {
                onPick(candidate.personId, target.roleId, target.date);
                onClose();
              }}
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="block text-[10.5px] text-faint">
                  {candidate.roleCountLast90}× in 90d
...
function explainNoCandidates(excludedCount: number): string {
  return excludedCount === 0
    ? 'No one in this region is eligible for this role.'
