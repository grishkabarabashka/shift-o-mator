...
export function projectCells(input: CellProjectionInput): CellProjection {
  ...
  for (const [key, compDayId] of proposedByCell) {
    ...
    else if (existing.kind === 'ROLE') byCell.set(key, { ...existing, proposedCompDay: compDayId });
  }
