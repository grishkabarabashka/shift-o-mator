 ...
 * `content` into `contentKind`/`roleId`/`marker`/`timeOverride` (an EF-mapped
...
export function regionFromWire(w: WireRegion): Region {
  return {
    id: w.id,
    name: w.name,
    primaryTimeZone: w.primaryTimeZone,
    primaryLocationId: w.primaryLocationId,
    locationIds: w.locationIds,
    compOffPolicy: {
      windowBeforeDays: w.compOffPolicy.windowBeforeDays,
      windowAfterDays: w.compOffPolicy.windowAfterDays,
      excludedWeekdays: weekdaysFromWire(w.compOffPolicy.excludedWeekdays),
      agingThresholdDays: w.compOffPolicy.agingThresholdDays,
      requiresApprovalWhenNoSlot: w.compOffPolicy.requiresApprovalWhenNoSlot,
    },
  };
}
...
export function unitFromWire(w: WirePlanningUnit): PlanningUnit {
  return {
    ...
    ...(w.regionId ? { regionId: w.regionId } : {}),
    groupBy: camelToUpperSnake<GroupBy>(w.groupBy),
  };
}

interface WireShiftDefinition {
  readonly id: string;
...
function roleRequirementFromWire(w: WireRoleRequirement): RoleRequirement {
...
export function dayConfigurationFromWire(w: WireDayConfiguration): DayConfiguration {
  return {
    ...
    roleRequirements: w.roleRequirements.map(roleRequirementFromWire),
  };
}

interface WireRoleEligibility {
  readonly roleId: string;
...
function eligibilityFromWire(w: WireRoleEligibility): RoleEligibility {
  return {
    roleId: w.roleId,
...
interface WirePerson {
  ...
  readonly eligibility: readonly WireRoleEligibility[];
}
...
export function personFromWire(w: WirePerson): Person {
  return {
    ...
    ...(w.defaultRoleId ? { defaultRoleId: w.defaultRoleId } : {}),
...
export function absenceCapacityRuleFromWire(w: WireAbsenceCapacityRule): AbsenceCapacityRule {
  return {
    id: w.id,
    regionId: w.regionId,
    scope: w.scopeRoleId ? { kind: 'ROLE_POOL', roleId: w.scopeRoleId } : { kind: 'REGION' },
...
export interface WireReferenceData {
  ...
  readonly shifts: readonly WireShiftDefinition[];
  readonly roles: readonly WireShiftRole[];
...
export function referenceFromWire(w: WireReferenceData): ReferenceData {
  return {
    ...
    regions: w.regions.map(regionFromWire),
    units: w.units.map(unitFromWire),
    shifts: w.shifts.map(shiftFromWire),
    roles: w.roles.map(roleFromWire),
