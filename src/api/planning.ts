...
export async function fetchCandidates(params: {
  ...
  return {
    available: wire.available.map((c) => ({
      personId: c.personId,
      name: c.name,
      roleCountLast90: c.roleCountLast90,
...
interface WireAutoPopulateResult {
  ...
  readonly gaps: readonly { readonly date: IsoDate; readonly roleId: RoleId; readonly code: string; readonly reason: string }[];
}
...
export async function runAutoPopulate(params: {
  ...
  return {
    changes,
    // `Code` here is a role code (e.g. "Batch-L"), not an enum — no case conversion.
    gaps: wire.gaps,
