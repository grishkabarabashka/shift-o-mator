...
///   1. eligibility  — the role is on the person's list;
...
/// Survivors sort by:
///   3. 90-day fairness — fewer times holding this role ranks higher;
...
public static class CandidateRanker
{
    ...
    public record Candidate(
        string PersonId, string Name, int RoleCountLast90, int? DaysSinceLastHeld,
    ...
    public record RankParams(
        string RoleId, DateOnly Date, string RegionId, DatasetIndex Index,
    ...
    public static CandidateResult Rank(RankParams p)
    {
        var pool = p.Index.People.Values
            .Where(person => person.RegionId == p.RegionId && person.IsIncluded && person.Eligibility.Any(e => e.RoleId == p.RoleId))
            .ToList();
        ...
        foreach (var person in pool)
        {
            ...
            var roleCountLast90 = own.Count(a =>
                a.ContentKind == AssignmentContentKind.Role && a.RoleId == p.RoleId && a.Date >= fairnessSince && a.Date < p.Date);

            var lastHeld = own
                .Where(a => a.ContentKind == AssignmentContentKind.Role && a.RoleId == p.RoleId && a.Date < p.Date)
                .Select(a => a.Date)
                .OrderBy(d => d)
                .Cast<DateOnly?>()
                .LastOrDefault();
            ...
            var eligibility = person.Eligibility.FirstOrDefault(e => e.RoleId == p.RoleId);
            ...
            available.Add(new Candidate(person.Id, person.DisplayName, roleCountLast90, daysSinceLastHeld, weekendLoad, warnings));
        }

        available = [.. available
            // Меньше — важнее: сначала кто реже держал роль в окне.
            .OrderBy(c => c.RoleCountLast90)
    ...
    /// default roles so there's one absence/comp-day/blackout/weekday rule, not two that
    /// eventually drift.
    /// </summary>
