...
public class DatasetIndex
{
    ...
    public required IReadOnlyDictionary<string, Region> Regions { get; init; }
    public required IReadOnlyDictionary<string, PlanningUnit> Units { get; init; }
    public required IReadOnlyDictionary<string, ShiftDefinition> Shifts { get; init; }
    public required IReadOnlyDictionary<string, ShiftRole> Roles { get; init; }
    public required IReadOnlyDictionary<string, Person> People { get; init; }
    public required IReadOnlyDictionary<string, IReadOnlyList<ShiftRole>> RolesByRegion { get; init; }
    public required IReadOnlyDictionary<string, IReadOnlyList<Person>> PeopleByRegion { get; init; }
    public required IReadOnlyDictionary<string, IReadOnlyList<Person>> PeopleByUnit { get; init; }
    public required IReadOnlyDictionary<string, IReadOnlyList<DayConfiguration>> DayConfigsByRegion { get; init; }
    ...
    public static DatasetIndex Build(ScheduleDataset data)
    {
        ...
        return new DatasetIndex
        {
            ...
            Regions = data.Regions.ToDictionary(x => x.Id),
            Units = data.Units.ToDictionary(x => x.Id),
            Shifts = data.Shifts.ToDictionary(x => x.Id),
            Roles = data.Roles.ToDictionary(x => x.Id),
