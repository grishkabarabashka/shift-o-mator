...
public static class DraftService
{
    ...
    private static void ValidateAssignmentDomain(Assignment a, DatasetIndex index)
    {
        ...
        if (a.ContentKind != AssignmentContentKind.Role) return; // markers carry no role

        if (a.RoleId is null) throw new DraftDomainException("ROLE_REQUIRED", "A role assignment needs a roleId.");
        if (!index.Roles.TryGetValue(a.RoleId, out var role))
            throw new DraftDomainException("ROLE_NOT_FOUND", $"Role {a.RoleId} does not exist.");

        // Point 4: a role belongs to a region; matching codes across regions are
        // coincidental (ADR-0004) — a person can never be handed a role from another region.
        if (role.RegionId != person.RegionId)
        {
            throw new DraftDomainException("ROLE_OUTSIDE_REGION",
                $"Role {role.Code} belongs to region {role.RegionId}, not {person.RegionId} (ADR-0004).");
        }
    }
        ...
        ScheduleDataset current, DatasetIndex index, DraftSession session, string actorId, DateTimeOffset now)
    {
        ...
        var touchedRegions = new HashSet<string>();
        ...
        foreach (var change in ordered.Where(c => c.TargetType == DraftTargetType.Assignment))
        {
            switch (change.Op)
            {
                case DraftOp.Create:
                {
                    ...
                    touchedRegions.Add(after.RegionId);
                    if (after.ContentKind == AssignmentContentKind.Role) liveAssignmentIdsThisPublish.Add(after.Id);
                    break;
                }
                case DraftOp.Update:
                {
                    ...
                    touchedDates.Add(after.Date);
                    touchedRegions.Add(actual.RegionId);
                    touchedRegions.Add(after.RegionId);
                    if (after.ContentKind == AssignmentContentKind.Role) liveAssignmentIdsThisPublish.Add(after.Id);
                    break;
                }
        ...
        if (touchedRegions.Count > 0)
        {
            ...
            foreach (var regionId in touchedRegions)
            {
                var cells = CoverageCalculator.Compute(regionId, rangeFrom, rangeTo, newAssignments, postIndex);
                var issues = Validator.Validate(new Validator.ValidateParams(
                    regionId, rangeFrom, rangeTo, newAssignments, newAbsences, newCompDays, cells,
