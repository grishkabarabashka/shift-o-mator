...
/// <summary>
/// Person identity/roster fields (name, region, unit, location, default shift, org
...
public static class PeopleAdminEndpoints
{
    public record PersonRequest(
        string DisplayName, string Initials, string? EmployeeId, string RegionId, string UnitId,
        string LocationId, string DefaultShiftId, OrgCategory OrgCategory, bool IsActive, bool IsIncluded);
    ...
    private static async Task<AdminValidation> ValidateAsync(PersonRequest req, ScheduleDbContext db, CancellationToken ct)
    {
        ...
        v.Require(nameof(req.DefaultShiftId), req.DefaultShiftId);
        ...
        if (!string.IsNullOrWhiteSpace(req.DefaultShiftId) && !await db.Shifts.AnyAsync(s => s.Id == req.DefaultShiftId, ct))
            v.Add(nameof(req.DefaultShiftId), $"Shift {req.DefaultShiftId} does not exist.");
