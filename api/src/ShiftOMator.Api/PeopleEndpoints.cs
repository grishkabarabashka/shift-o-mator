...
{
    public record RoleEligibilityRequest(string RoleId, double TargetShare, int? MinPerWeek, int? MaxPerWeek);
    ...
    public record UpdatePersonRequest(
        List<RoleEligibilityRequest> Eligibility,
    ...
    public static void MapPeopleEndpoints(this WebApplication app)
    {
        app.MapPut("/api/people/{id}", async (string id, UpdatePersonRequest req, ScheduleDbContext db, CancellationToken ct) =>
        {
            ...
            {
                person.Eligibility.Add(new RoleEligibility
                {
                    PersonId = id,
                    RoleId = e.RoleId,
            ...
            person.DefaultRoleId = req.DefaultRoleId;
