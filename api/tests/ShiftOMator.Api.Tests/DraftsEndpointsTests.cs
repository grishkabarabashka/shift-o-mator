...
public class DraftsEndpointsTests(ApiTestFactory factory)
{
    ...
    private const string RoleId = "AMER:Crew";
    private const string RegionId = "AMER";
    ...
    private static JsonElement AssignmentPayload(string id, string personId, DateOnly date, string roleId) =>
    ...
    public async Task Publish_applies_the_assignment_and_writes_history()
    {
        ...
        var appendResponse = await client.PostAsJsonAsync($"/api/drafts/{draftId}/changes", new
        {
            ...
            after = AssignmentPayload(assignmentId, PersonId, date, RoleId),
        });
    ...
    public async Task Second_draft_publishing_the_same_cell_gets_409_with_typed_conflicts()
    {
        ...
        {
            targetType = "assignment", op = "create", entityId = idA, after = AssignmentPayload(idA, PersonId, date, RoleId),
        })).EnsureSuccessStatusCode();

        (await client.PostAsJsonAsync($"/api/drafts/{draftB}/changes", new
        {
            targetType = "assignment", op = "create", entityId = idB, after = AssignmentPayload(idB, PersonId, date, RoleId),
        })).EnsureSuccessStatusCode();
    ...
    [Fact]
    public async Task Appending_a_role_from_another_region_is_rejected_with_a_typed_error()
    {
        var client = factory.CreateClient();
        var date = NextDate();
        var assignmentId = $"as-test-{Guid.NewGuid():n}";
        ...
        var body = await response.Content.ReadFromJsonAsync<JsonElement>();
        Assert.Equal("ROLE_OUTSIDE_REGION", body.GetProperty("code").GetString());
    }
    ...
    public async Task Discard_leaves_the_plan_untouched()
    {
        ...
        {
            targetType = "assignment", op = "create", entityId = assignmentId, after = AssignmentPayload(assignmentId, PersonId, date, RoleId),
        })).EnsureSuccessStatusCode();
