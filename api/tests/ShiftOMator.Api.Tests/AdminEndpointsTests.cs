...
public class AdminEndpointsTests(ApiTestFactory factory) : IDisposable
{
    ...
    // A Planner-role stub (the shared collection's default) must not get Admin
    ...
    public async Task Planner_is_also_forbidden_from_admin_endpoints()
    {
        ...
        var response = await client.GetAsync("/api/admin/regions");
    ...
    public async Task In_place_edit_mutates_a_role_color_directly()
    {
        ...
        var before = await client.GetFromJsonAsync<JsonElement>("/api/admin/roles/AMER:Lead");
        var newColor = "#123456";

        var response = await client.PutAsJsonAsync("/api/admin/roles/AMER:Lead", new
        {
            regionId = before.GetProperty("regionId").GetString(),
        ...
        var after = await client.GetFromJsonAsync<JsonElement>("/api/admin/roles/AMER:Lead");
        ...
        await client.PutAsJsonAsync("/api/admin/roles/AMER:Lead", new
        {
            regionId = before.GetProperty("regionId").GetString(),
            code = before.GetProperty("code").GetString(),
            label = before.GetProperty("label").GetString(),
    ...
    {
        // GET /api/admin/roles/{id} isn't mapped as a single-resource route (only the
        // list + PUT/POST/DELETE are) — confirm the list contains the fixture role used
        ...
        Assert.Contains(roles.EnumerateArray(), r => r.GetProperty("id").GetString() == "AMER:Lead");
    }
    ...
    public async Task New_day_configuration_version_does_not_touch_the_old_versions_data()
    {
        ...
        var originalRequirements = original.GetProperty("roleRequirements").EnumerateArray()
            .Select(r => (r.GetProperty("roleId").GetString(), r.GetProperty("min").GetInt32()))
            .OrderBy(x => x.Item1)
            .ToList();
        ...
        var response = await client.PostAsJsonAsync("/api/admin/day-configurations", new
        {
            ...
            roleRequirements = new[]
            {
                new { roleId = "AMER:Lead", min = 99, max = (int?)null, isDefault = false,
                      timingOverrideStart = (TimeOnly?)null, timingOverrideEnd = (TimeOnly?)null, timingOverrideCrossesMidnight = (bool?)null },
                new { roleId = "AMER:Crew", min = 1, max = (int?)null, isDefault = true,
                      timingOverrideStart = (TimeOnly?)null, timingOverrideEnd = (TimeOnly?)null, timingOverrideCrossesMidnight = (bool?)null },
            },
        });
        response.EnsureSuccessStatusCode();
        ...
        Assert.Equal(99, created.GetProperty("roleRequirements").EnumerateArray()
            .First(r => r.GetProperty("roleId").GetString() == "AMER:Lead").GetProperty("min").GetInt32());
        ...
        try
        {
            ...
            var stillRequirements = stillOriginal.GetProperty("roleRequirements").EnumerateArray()
                .Select(r => (r.GetProperty("roleId").GetString(), r.GetProperty("min").GetInt32()))
                .OrderBy(x => x.Item1)
                .ToList();
    ...
    [Fact]
    public async Task Day_configuration_creation_rejects_a_role_outside_the_target_region()
    {
        using var admin = AdminFactory();
        var client = admin.CreateClient();
        ...
        var response = await client.PostAsJsonAsync("/api/admin/day-configurations", new
        {
            ...
            roleRequirements = new[]
            {
                new { roleId = "EMEA:Lead", min = 1, max = (int?)null, isDefault = false,
                      timingOverrideStart = (TimeOnly?)null, timingOverrideEnd = (TimeOnly?)null, timingOverrideCrossesMidnight = (bool?)null },
            },
        });
        ...
        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
        var body = await response.Content.ReadFromJsonAsync<JsonElement>();
        Assert.True(body.GetProperty("errors").TryGetProperty("roleRequirements[0].roleId", out _));
    }
