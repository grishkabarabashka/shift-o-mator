...
public class CoverageCalculatorTests
{
    private static readonly DayConfiguration Weekday = MakeDayConfig(
        ...
        roleRequirements: [new RoleRequirement { DayConfigurationId = "", RoleId = LeadRole.Id, Min = 1, Max = 3, IsDefault = true }]);
    ...
    public class ComputeOverAPeriod
    {
        ...
        private static List<CoverageCell> CoverageFor(params (string PersonId, string Date)[] placements)
        {
            ...
            return CoverageCalculator.Compute(TestRegion.Id, new DateOnly(2026, 9, 7), new DateOnly(2026, 9, 13), data.Assignments, BuildIndex(data));
        }
        ...
        public void Roster_markers_do_not_count_toward_coverage()
        {
            var data = MakeDataset(
                ...
                dayConfigurations: [Weekday, Weekend, HolidayConfig]);
            var cells = CoverageCalculator.Compute(TestRegion.Id, new DateOnly(2026, 9, 8), new DateOnly(2026, 9, 8), data.Assignments, BuildIndex(data));
        ...
        {
            var shadow = new ShiftRole
            {
                Id = "r-shadow",
                RegionId = NightRole.RegionId,
                Code = "Shadow",
            ...
            var config = MakeDayConfig(
                ...
                roleRequirements:
                [
                    new RoleRequirement { DayConfigurationId = "", RoleId = LeadRole.Id, Min = 1, IsDefault = true },
                    new RoleRequirement { DayConfigurationId = "", RoleId = shadow.Id, Min = 1, IsDefault = false },
                ]);
            var data = MakeDataset(
                people: People,
            ...
            Assert.Single(cells);
            Assert.Equal(LeadRole.Id, cells[0].RoleId);
            Assert.Equal(0, cells[0].Actual);
        }
