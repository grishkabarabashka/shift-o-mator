...
public class DayConfigurationResolverTests
{
    private static readonly DayConfiguration MonThu = MakeDayConfig(
        ...
        roleRequirements: [new RoleRequirement { DayConfigurationId = "", RoleId = LeadRole.Id, Min = 1, Max = 1, IsDefault = true }]);
    ...
    private static readonly DayConfiguration Friday = MakeDayConfig(
        ...
        roleRequirements: [new RoleRequirement { DayConfigurationId = "", RoleId = NightRole.Id, Min = 2, IsDefault = true }]);
    ...
    private static readonly DayConfiguration Weekend = MakeDayConfig(
        ...
        roleRequirements: [new RoleRequirement { DayConfigurationId = "", RoleId = LeadRole.Id, Min = 1, IsDefault = true }]);

    private static readonly DayConfiguration Holiday = MakeDayConfig(
        "dc-holiday", DayConfigKey.Holiday,
        weekdays: [],
        roleRequirements: [new RoleRequirement { DayConfigurationId = "", RoleId = NightRole.Id, Min = 1, IsDefault = true }]);
    ...
    public class GroupSelection
    {
        ...
        public void Monday_falls_into_weekday()
        {
            ...
            Assert.Equal("dc-weekday", DayConfigurationResolver.Resolve(TestRegion.Id, new DateOnly(2026, 9, 7), _index)?.Id);
        }
        ...
        {
            // 2026-09-11 — пятница.
            var config = DayConfigurationResolver.Resolve(TestRegion.Id, new DateOnly(2026, 9, 11), _index);
            Assert.Equal("dc-friday", config?.Id);
            Assert.Equal(NightRole.Id, config?.RoleRequirements[0].RoleId);
        }

        [Fact]
        public void Weekend_is_separate_from_weekdays()
        {
            Assert.Equal("dc-weekend", DayConfigurationResolver.Resolve(TestRegion.Id, new DateOnly(2026, 9, 12), _index)?.Id);
        }
        ...
        public void Holiday_overrides_a_weekday()
        {
            ...
            Assert.Equal("dc-holiday", DayConfigurationResolver.Resolve(TestRegion.Id, new DateOnly(2026, 9, 8), _index)?.Id);
        }
        ...
        public void Holiday_status_is_judged_by_the_region_primary_location_not_the_person()
        {
            ...
            Assert.Equal("dc-weekday", DayConfigurationResolver.Resolve(TestRegion.Id, new DateOnly(2026, 9, 8), withPuneHoliday)?.Id);
        }
        ...
        public void No_matching_group_means_no_requirements()
        {
            ...
            Assert.Null(DayConfigurationResolver.Resolve(TestRegion.Id, new DateOnly(2026, 9, 12), weekdaysOnly));
        }
    }
    ...
    public class EffectiveDatedVersioning
    {
        ...
        private static readonly DayConfiguration V2 = MakeDayConfig(
            ...
            roleRequirements: [new RoleRequirement { DayConfigurationId = "", RoleId = LeadRole.Id, Min = 2, IsDefault = true }]);
        ...
        {
            Assert.Equal(1, DayConfigurationResolver.ResolveRequirement(TestRegion.Id, LeadRole.Id, new DateOnly(2026, 3, 2), _index)?.Min);
        }

        [Fact]
        public void After_the_effective_date_the_new_version_applies()
        {
            Assert.Equal(2, DayConfigurationResolver.ResolveRequirement(TestRegion.Id, LeadRole.Id, new DateOnly(2026, 9, 2), _index)?.Min);
        }

        [Fact]
        public void On_the_effective_date_the_new_version_already_applies()
        {
            Assert.Equal(2, DayConfigurationResolver.ResolveRequirement(TestRegion.Id, LeadRole.Id, new DateOnly(2026, 9, 1), _index)?.Min);
        }
        ...
        public void A_future_version_does_not_affect_today()
        {
            var future = MakeDayConfig(
                ...
                roleRequirements: [new RoleRequirement { DayConfigurationId = "", RoleId = LeadRole.Id, Min = 9, IsDefault = true }]);
            var withFuture = IndexWith([V1, V2, future]);
            Assert.Equal(2, DayConfigurationResolver.ResolveRequirement(TestRegion.Id, LeadRole.Id, new DateOnly(2026, 9, 2), withFuture)?.Min);
        }
    }
