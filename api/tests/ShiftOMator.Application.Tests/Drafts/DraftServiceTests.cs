...
public class DraftServiceTests
{
    ...
    private static Assignment NewAssignment(DateOnly date, int version = 0, string id = "as-1") => new()
    {
        ...
        PersonId = "p-1",
        Date = date,
        RegionId = TestFixtures.TestRegion.Id,
        ContentKind = AssignmentContentKind.Role,
        RoleId = TestFixtures.LeadRole.Id,
    ...
    [Fact]
    public void Append_create_rejects_a_role_belonging_to_another_region()
    {
        var role = new ShiftRole
        {
            Id = "r-other", RegionId = "R2", Code = "Lead", Label = "Lead", Color = "#000",
            TimeZone = "America/New_York", Start = new TimeOnly(9, 0), End = new TimeOnly(17, 0),
        };
        var person = TestFixtures.MakePerson("p-1");
        var dataset = TestFixtures.MakeDataset(people: [person], roles: [TestFixtures.LeadRole, role]);
        ...
        Assert.Equal("ROLE_OUTSIDE_REGION", ex.Code);
    }
    ...
    public void Publish_computes_remaining_gaps_from_the_post_publish_coverage()
    {
        var dayConfig = TestFixtures.MakeDayConfig(
            ...
            roleRequirements: [new RoleRequirement { DayConfigurationId = "dc-1", RoleId = TestFixtures.LeadRole.Id, Min = 2 }]);
