...
public class AutoPopulateServiceTests
{
    ...
    private static readonly DayConfiguration WeekendConfig = MakeDayConfig(
        ...
        roleRequirements: [new RoleRequirement { DayConfigurationId = "", RoleId = NightRole.Id, Min = 1, IsDefault = false }]);
        ...
        List<Person>? people = null, List<Assignment>? assignments = null, IReadOnlySet<string>? locked = null)
    {
        ...
        return AutoPopulateService.Run(new AutoPopulateService.Params(
            TestRegion.Id, RangeFrom, RangeTo, locked ?? new HashSet<string>(),
    ...
    public class Defaults
    {
        ...
        public void Places_DefaultRoleId_on_the_persons_weekdays()
        {
            ...
            [
                new RoleEligibility { PersonId = "", RoleId = LeadRole.Id, TargetShare = 1 },
                new RoleEligibility { PersonId = "", RoleId = NightRole.Id, TargetShare = 0 },
            ]);
            var result = Run(people: [alice]);

            var weekdayCreates = result.Assignments.Where(a => a.RoleId == LeadRole.Id).ToList();
        ...
        public void A_mismatched_default_is_not_used_in_pass_A()
        {
            ...
            var bob = MakePerson("p-bob", defaultRoleId: NightRole.Id,
                eligibility: [new RoleEligibility { PersonId = "", RoleId = NightRole.Id, TargetShare = 1 }]);
            var result = Run(people: [bob]);

            Assert.DoesNotContain(result.Assignments, a => a.RoleId == LeadRole.Id);
            Assert.Contains(result.Gaps, g => g.RoleId == LeadRole.Id);
        }
    }
    ...
    public class RankedFill
    {
        ...
        {
            var alice = MakePerson("p-alice", eligibility: [new RoleEligibility { PersonId = "", RoleId = NightRole.Id, TargetShare = 1 }]);
            var result = Run(people: [alice]);
        ...
        public void Leaves_a_gap_with_a_reason_when_no_one_can_close_it()
        {
            ...
            var gap = result.Gaps.FirstOrDefault(g => g.RoleId == NightRole.Id);
            Assert.NotNull(gap);
    ...
    public class Determinism
    {
        ...
        public void The_same_input_gives_the_same_set_of_changes()
        {
            ...
            [
                new RoleEligibility { PersonId = "", RoleId = LeadRole.Id, TargetShare = 1 },
                new RoleEligibility { PersonId = "", RoleId = NightRole.Id, TargetShare = 1 },
            ]);
            var bob = MakePerson("p-bob", eligibility: [new RoleEligibility { PersonId = "", RoleId = NightRole.Id, TargetShare = 1 }]);
